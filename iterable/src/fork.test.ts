...
import {take} from './operator/terminal/take'
...
test('fork/two forks one returns immediately', async (t) => {
  t.plan(1)
  ...
  const v = fromCombineLatest([fork1, fork2])
...
test('fork/two forks one returns before end', async (t) => {
  t.plan(1)

  const it: Iterator<number> = [1, 2, 3, 4][Symbol.iterator]()
  ...
  t.deepEquals(result.value, [
    ...
    {value: [4, 2], previous: [3, 2], index: 0},
  ])
})
